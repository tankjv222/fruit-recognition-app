...
# ─────────────────────────────────────────────
#  TẢI MODEL (cache để không reload mỗi lần)
# ─────────────────────────────────────────────
...
with st.spinner("Đang tải mô hình AI..."):
    try:
        ...
        st.error("⚠️ Không tìm thấy file **best.pt**. Hãy đặt file model cùng thư mục với app.py rồi chạy lại.")
        st.stop()
...
if uploaded_file is not None:
    ...
    if top1_conf < 60:
        st.warning("⚠️ Độ tự tin thấp — hãy thử ảnh rõ hơn, chụp gần trái cây hơn.")
